...
if __name__ == "__main__":    
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        train_ppl, train_kld = train_step(epoch, args.batch_size, args.use_cuda and t.cuda.is_available(), args.dropout)
        train_ppl = train_ppl.data.cpu().numpy()[0]
        train_kld = train_kld.data.cpu().numpy()[0]

        valid_ppl, valid_kld = validate(args.batch_size, args.use_cuda and t.cuda.is_available())
        valid_ppl = valid_ppl.data.cpu().numpy()[0]
        valid_kld = valid_kld.data.cpu().numpy()[0]

        ppl_result += [valid_ppl]
        kld_result += [valid_kld]

        is_best = valid_ppl < best_ppl
        best_ppl = min(valid_ppl, best_ppl)

        print('[%s]---TRAIN-ppl[%s]kld[%s]------VALID-ppl[%s]kld[%s]'%(epoch, train_ppl, train_kld, valid_ppl, valid_kld))
